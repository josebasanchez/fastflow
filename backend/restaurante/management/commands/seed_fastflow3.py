...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        cola_specs = [
            ...
            (0, time(10, 30), timedelta(minutes=-45), Pedido.ESTADO_PENDIENTE, None, "late"),
            (1, time(11, 0), timedelta(minutes=60), Pedido.ESTADO_PENDIENTE, None, "lt1h"),
            (2, time(11, 30), timedelta(minutes=110), Pedido.ESTADO_PENDIENTE, None, "lt2h"),
            (3, time(12, 0), timedelta(minutes=170), Pedido.ESTADO_PENDIENTE, None, "lt3h"),
        ...
        # -------------------------
        # 3) Pendientes del dia anterior (servicio de cena)
        # -------------------------
        prev_date = anchor_date - timedelta(days=1)
        prev_specs = [
            # (mesa_idx, hora_reserva, deadline_delta)
            (0, time(20, 0), timedelta(minutes=45)),
            (1, time(20, 30), timedelta(minutes=60)),
            (2, time(21, 0), timedelta(minutes=75)),
        ]
        prev_anchor = aware(datetime.combine(prev_date, time(19, 30)))

        for idx, (mesa_idx, hora_reserva, delta_minutes) in enumerate(prev_specs):
            mesa = mesas[mesa_idx % len(mesas)]
            nombre, email = pick_cliente(9500 + idx)
            reserva_dt = aware(datetime.combine(prev_date, hora_reserva))
            reserva = get_or_create_reserva(
                mesa=mesa,
                fecha=prev_date,
                hora=hora_reserva,
                ...
                estado=Reserva.ESTADO_CONFIRMADA,
                pedido_anticipado=True,
                creado_en=prev_anchor - timedelta(hours=rng.randint(1, 4)),
            )

            plato = rng.choice(platos)
            cantidad = rng.choices([1, 2], weights=[0.8, 0.2])[0]
            fecha_limite = reserva_dt - delta_minutes
            pedido_creado_en = fecha_limite - timedelta(minutes=rng.randint(10, 80))
        ...
        mesas_a_reservar = (
            Reserva.objects.filter(fecha__in=[anchor_date, prev_date])
