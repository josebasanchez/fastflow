...
export default function MesasDisponibles({ auth, onRequireLogin, mesas, filtros, setFiltros, selectedMesaId, onSelectMesa, occupiedSlotsByMesa = {}, onContinue, apiBaseUrl }) {
  ...
  return (
    <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-[#d9cdb8] dark:bg-slate-900 dark:ring-slate-800">
      <h2 className="text-lg font-semibold text-[#5c3d1e] dark:text-slate-100">Paso 1 · Buscar y elegir mesa</h2>
      {!auth ? <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">Sin login puedes consultar mesa y horarios. Para reservar, inicia sesion como cliente.</p> : null}

      <div className="mt-3 grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950">
          <div className="relative overflow-hidden rounded-xl border border-slate-300 bg-transparent dark:border-slate-700 dark:bg-transparent">
          ...
          {mesasSinPosicion.length > 0 ? (
            <div className="mt-3">
              <p className="mb-2 text-xs text-slate-600 dark:text-slate-300">Mesas sin posicion en el plano, puedes seleccionarlas aqui:</p>
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {mesasSinPosicion.map((m) => (
                  <button key={m.id} type="button" onClick={() => onSelectMesa?.(m)} className={`rounded-lg border px-3 py-2 text-left text-sm dark:text-slate-100 ${Number(selectedMesaId) === m.id ? "border-indigo-600 bg-indigo-50 dark:bg-indigo-950/40" : "border-slate-300 bg-white dark:border-slate-700 dark:bg-slate-900"}`}>
        ...
        <aside className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950">
          <div className="mb-2 flex items-center justify-between">
            <button
              type="button"
              onClick={() => setCalendarMonth((prev) => new Date(prev.getFullYear(), prev.getMonth() - 1, 1))}
              className="rounded-md border border-slate-300 bg-white px-2 py-1 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
            >
              {"<"}
            </button>
            <p className="text-sm font-semibold capitalize text-[#5c3d1e] dark:text-slate-100">{monthLabel}</p>
            <button
              type="button"
              onClick={() => setCalendarMonth((prev) => new Date(prev.getFullYear(), prev.getMonth() + 1, 1))}
              className="rounded-md border border-slate-300 bg-white px-2 py-1 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
          ...
          <div className="mb-1 grid grid-cols-7 gap-1 text-center text-xs text-slate-500 dark:text-slate-400">
            {["L", "M", "X", "J", "V", "S", "D"].map((d) => (
              <span key={d}>{d}</span>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-1">
            {calendarDays.map((date, idx) => {
              ...
              const dayClass = isPast
                ? "border-slate-200 bg-slate-200 text-slate-500 cursor-not-allowed dark:border-slate-800 dark:bg-slate-800 dark:text-slate-500"
                : isSelected
                  ? "border-indigo-600 bg-indigo-600 text-white"
                  : isFullDay
                    ? "border-red-300 bg-red-100 text-red-800 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-200"
                    : "border-transparent bg-white text-slate-700 dark:bg-slate-900 dark:text-slate-100";
              ...
              return (
                <button
                  ...
                  className={`h-8 rounded-md border text-sm ${dayClass} ${isToday ? "font-bold" : "font-normal"} ${isPast ? "" : "hover:bg-indigo-50 dark:hover:bg-indigo-950/40"}`}
                >
                  {date.getDate()}
                </button>
              );
          ...
          <div className="mt-3 flex items-end gap-2">
            <div className="flex flex-1 flex-col items-center">
              <div
                className="relative h-16 w-full max-w-[240px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md ring-1 ring-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:ring-slate-800"
          ...
          onPointerCancel={(e) => {
              ...
              >
                <div className="pointer-events-none absolute left-3 top-1/2 z-20 -translate-y-1/2 text-slate-400 dark:text-slate-500">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d="M15 18l-6-6 6-6" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
                <div className="pointer-events-none absolute right-3 top-1/2 z-20 -translate-y-1/2 text-slate-400 dark:text-slate-500">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d="M9 18l6-6-6-6" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
                <div className="pointer-events-none absolute left-3 top-2 z-20 text-slate-400 dark:text-slate-500">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                ...
                <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center pt-2 text-4xl font-semibold text-slate-800 drop-shadow-sm transition-transform dark:text-slate-100" style={{ transform: `translateX(${dragX}px)` }}>
                  {capacidad}
                </div>
                <div
                  className="pointer-events-none absolute inset-0 z-0 flex items-center justify-center pt-2 text-4xl font-semibold text-slate-800 opacity-40 dark:text-slate-100"
                  style={{
          ...
          <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
            <div className="mt-3 space-y-3">
              {mesasPermitidas.map((mesa) => {
                ...
                return (
                  <div key={`slots-${mesa.id}`} className="rounded-lg border border-slate-200 p-2 dark:border-slate-800">
                    <p className="mb-2 text-sm font-medium text-[#5c3d1e] dark:text-slate-100">{prettyMesaName(mesa.nombre)}</p>
                    {loading ? (
                      <p className="text-xs text-slate-500 dark:text-slate-400">Cargando...</p>
                    ) : slots.length === 0 ? (
                      <p className="text-xs text-slate-500 dark:text-slate-400">Sin huecos disponibles desde la hora actual.</p>
                    ) : (
                      <div className="grid max-h-36 grid-cols-3 gap-2 overflow-auto rounded-lg">
                        {slots.map((slot) => {
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-lg px-2 py-1 text-sm ${selected ? "bg-indigo-600 text-white" : "bg-slate-100 dark:bg-slate-800 dark:text-slate-100"} ${occupied ? "line-through bg-slate-200 text-slate-500 dark:bg-slate-950 dark:text-slate-500" : ""} ${!isCliente || occupied ? "cursor-not-allowed opacity-60" : "hover:bg-slate-200 dark:hover:bg-slate-700"}`}
                            >
                              {slot}
                            </button>
                          );
                    ...
                    {!hasFreeSlot && slots.length > 0 ? (
                      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">Todos los horarios visibles estan ocupados.</p>
                    ) : null}
                  </div>
                );
              })}
            </div>
            {!isCliente ? <p className="mt-1 text-xs text-amber-700 dark:text-amber-200">Inicia sesion para seleccionar un horario.</p> : null}
